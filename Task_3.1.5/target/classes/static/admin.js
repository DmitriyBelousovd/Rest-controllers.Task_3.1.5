...
async function showAllUsers() {
    const users = await getUsers('http://localhost:8082/api/admin')
...
async function addNewUser() {
    ...
    form.addEventListener('submit', async function (e) {
        ...
        urlGetAddedUser = `http://localhost:8082/api/admin/${formDataObject.username}`;
...
function deleteUser() {
    ...
    document.getElementById('allUsersTbody').addEventListener('click', async function (event) {
        if (event.target.classList.contains('btn-delete')) {
            ...
            userID = userId;
            urlDelete = `http://localhost:8082/api/admin/${userId}`;
            let user = await getUsers(`http://localhost:8082/api/admin/get/${userId}`);
...
async function rolesSelector() {
    ...
    const response = await fetch('http://localhost:8082/api/admin/roles');
    if (!response.ok) {
